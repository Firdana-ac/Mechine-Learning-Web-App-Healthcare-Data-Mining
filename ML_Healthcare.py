...
st.sidebar.title("Dataset dan Classifier")

dataset_name=st.sidebar.selectbox("Pilih Dataset: ",('Serangan Jantung',"Kanker Payudara"))
classifier_name = st.sidebar.selectbox("Pilih Classifier: ",("Logistic Regression","KNN","SVM","Decision Trees",
                                                              "Random Forest","Gradient Boosting","XGBoost"))

LE=LabelEncoder()
def get_dataset(dataset_name):
    if dataset_name=="Serangan Jantung":
...
def selected_dataset(dataset_name):
    ...
    elif dataset_name == "Kanker Payudara":
...
st.write(data)
st.write("Bentuk kumpulan data: ",data.shape)
...
def add_parameter_ui(clf_name):
    ...
    if clf_name == "Logistic Regression":
        ...
        SS = st.sidebar.slider("min_samples_split",1.0,10.0) 
        params["M"] = M
        params["C"] = C
        params["SS"] = SS
...
def compute(Y_pred,Y_test):
    ...
    plt.title("Nilai Prediksi vs Nilai Nyata")
    plt.legend()
    plt.grid(True)
    c1.pyplot()
    ...
    st.subheader("Metrics dari model: ")
    st.text('Precision: {} \nRecall: {} \nF1-Score: {} \nAccuracy: {} %\nMean Squared Error: {}'.format(
        round(precision, 3), round(recall, 3), round(fscore,3), round((acc*100),3), round((mse),3)))

st.markdown("<hr>",unsafe_allow_html=True)
st.header(f"1) Model Prediksi {dataset_name}")
st.subheader(f"Pengklasifikasian yang Digunakan: {classifier_name}")
...
def user_inputs_ui(dataset_name,data):
    user_val = {}
    if dataset_name == "Kanker Payudara":
        X = data.drop(["id","diagnosis"], axis=1)
    ...
    elif dataset_name == "Serangan Jantung":
        X = data.drop(["output"], axis=1)
        for col in X.columns:
            name=col
            col = st.number_input(col, abs(X[col].min()-round(X[col].std())), abs(X[col].max()+round(X[col].std())))
...
with st.expander("See more"):
    st.markdown("""
    Di bagian ini Anda dapat menggunakan nilai Anda sendiri untuk memprediksi variabel target.
     Masukkan nilai yang diperlukan di bawah ini dan Anda akan mendapatkan status berdasarkan nilai tersebut. <br>
    <p style='color: red;'> 1 - Risiko Tinggi </p> <p style='color: green;'> 0 - Risiko Rendah </p>
...
def user_predict():
    ...
    if U_pred == 0:
        st.write(U_pred[0], " - Anda tidak berisiko tinggi :)")
    else:
        st.write(U_pred[0], " - Anda berisiko tinggi :(")
